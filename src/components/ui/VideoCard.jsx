    ...
    >
      ...
      {video.type === 'embed' && !hasEmbed && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 border border-dashed border-white/10 text-ink-faint">
          ...
          <span className="text-xs font-medium">Add YouTube / Vimeo embed</span>
        </div>
      )}
      ...
      <div className="absolute inset-x-0 bottom-0 flex flex-col gap-2 p-4 sm:p-5">
        <div className="flex items-center justify-between gap-2">
          <h3 className="font-display text-sm font-semibold text-ink sm:text-base">{video.title}</h3>
        ...
        <div className="grid grid-rows-[0fr] transition-all duration-500 ease-out group-hover:grid-rows-[1fr]">
          <div className="overflow-hidden">
            <p className="pt-1 text-xs italic leading-relaxed text-ink-muted sm:text-sm">{video.hook}</p>
            <div className="mt-2 flex items-center gap-2">
              ...
              <span className="text-[10px] font-medium text-ink-faint">{video.retention}% retention</span>
            </div>
          </div>
        </div>
      </div>
